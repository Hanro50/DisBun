...
public class DeathMessageHandler extends DisBunModule {
	...
	private void Display(DeathMessage base, ServerInfo serv) {
		if (enabled) {
			...
			String Disc = String.format("EXP: %2f", base.XP);
			String Foot = "(" + base.Deathpos.tostring() + ")";
			Debug.out("sending stuff");
					...
					if (CHL != null) {
						try {
							MessageEmbed E = new EmbedBuilder()
									.setAuthor(String.format(BaseStr, vicName, AtkName, "[*"+Weapon+"*]"), null, ICON)
									.setColor(Clr).setDescription(Disc).setFooter(Foot).build();
