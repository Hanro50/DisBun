...
import org.han.spigot.Advancement;
import org.han.spigot.DeathMessage;
...
public class Events implements Listener {
	...
	@EventHandler
	public void PermCheck(PermissionCheckEvent e) {
		if (BPlugin.Config.isStockPerms()) {
			Debug.rep("Checking perms for:" + e.getSender().getName());
			if (e.getPermission().startsWith(PermKeys.KEY)) {
				Debug.rep("Checking DisBun perms");
				if (e.getSender() instanceof ProxiedPlayer) {
					CommandSender player = e.getSender();
					Debug.rep("UUID for checked party:" + ((ProxiedPlayer) player).getUniqueId());
					String ID = LinkUp.GetDiscordID(((ProxiedPlayer) player).getUniqueId());
					player.setPermission(PermKeys.LINK, (ID != null));
					if (ID == null) {
						return;
					}
					Member mem = GetDetails.getGuild().retrieveMemberById(ID).complete();
					player.setPermission(PermKeys.MEMBER, (mem != null));
					if (mem == null)
						return;
					for (Role role : GetDetails.getGuild().getRoles()) {
						player.setPermission(PermKeys.KEY + role.getName(), mem.getRoles().contains(role));
					}
					player.setPermission(PermKeys.BOOSTER, mem.getTimeBoosted() != null);
